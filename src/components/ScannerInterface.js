...
const ScannerInterface = () => {
  ...
  const ResultDisplay = ({ data }) => {
    ...
    const hasData = data.productName || data.quantity || data.description || (data.ingredients && Array.isArray(data.ingredients) && data.ingredients.length > 0) || (data.nutritionFacts && Array.isArray(data.nutritionFacts) && data.nutritionFacts.length > 0);
